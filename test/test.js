...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterCeil2, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterCeil2( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which rounds each iterated value to the nearest power of two toward positive infinity', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterCeil2( linspace( -200.0, 200.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( -200.0, 200.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = ceil2( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which rounds each iterated value to the nearest power of two toward positive infinity (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': ceil2( values[ 0 ] ),
			'done': false
		},
		{
			'value': ceil2( values[ 1 ] ),
			'done': false
		},
		{
			'value': ceil2( values[ 2 ] ),
			'done': false
		},
		{
			'value': ceil2( values[ 3 ] ),
			'done': true
		}
	];
